package com.zhidao.kstb.model.tabc;

public class CZRK_CSSQSPXX {
    private Long xlh;

    private Long spglxlh;

    private String sqh;

    private String gmsfhm;

    private String xm;

    private String xb;

    private String mz;

    private String csrq;

    private String cssj;

    private String csdgj;

    private String csdssx;

    private String csdxz;

    private String jhryhm;

    private String jhryxm;

    private String jhrygx;

    private String jhrehm;

    private String jhrexm;

    private String jhregx;

    private String fqhm;

    private String fqxm;

    private String mqhm;

    private String mqxm;

    private String jggj;

    private String jgssx;

    private String xx;

    private String xxjb;

    private String hh;

    private String csdjlb;

    private String cszmbh;

    private String sbrgmsfhm;

    private String sbrxm;

    private String sbrq;

    private String sblb;

    private String sbly;

    private String sldwjgdm;

    private String sldwmc;

    private String slrxm;

    private String slsj;

    private Long rkxlh;

    private String ssxq;

    private String pcsjgdm;

    private String pcsdm;

    private String xzjddm;

    private String spqk;

    private String spdwjgdm;

    private String spdwmc;

    private String sprxm;

    private String spsj;

    private String spjg;

    private String lxdh;

    private String shqk;

    private String shdwjgdm;

    private String shdwmc;

    private String shrxm;

    private String shrq;

    private Short blqk;

    private String qsrq;

    private Short shspzt;

    private String ywlx;

    private String shsplc;

    private String spyyms;

    private String spbh;

    private String spjb;

    private Long sppzxlh;

    private String bz;

    private String shspyj;

    private String dzbm;

    private String ysqrgx;

    private String xmpy;

    private String ywblsj;

    private String fxjdm;

    private String slrid;

    private String sprid;

    private String shrid;

    private String dqm;

    private String bdywswbh;

    private String ospjb;

    private Short qctbs;

    private String hzXm;

    private String hzGmsfhm;

    public Long getXlh() {
        return xlh;
    }

    public void setXlh(Long xlh) {
        this.xlh = xlh;
    }

    public Long getSpglxlh() {
        return spglxlh;
    }

    public void setSpglxlh(Long spglxlh) {
        this.spglxlh = spglxlh;
    }

    public String getSqh() {
        return sqh;
    }

    public void setSqh(String sqh) {
        this.sqh = sqh == null ? null : sqh.trim();
    }

    public String getGmsfhm() {
        return gmsfhm;
    }

    public void setGmsfhm(String gmsfhm) {
        this.gmsfhm = gmsfhm == null ? null : gmsfhm.trim();
    }

    public String getXm() {
        return xm;
    }

    public void setXm(String xm) {
        this.xm = xm == null ? null : xm.trim();
    }

    public String getXb() {
        return xb;
    }

    public void setXb(String xb) {
        this.xb = xb == null ? null : xb.trim();
    }

    public String getMz() {
        return mz;
    }

    public void setMz(String mz) {
        this.mz = mz == null ? null : mz.trim();
    }

    public String getCsrq() {
        return csrq;
    }

    public void setCsrq(String csrq) {
        this.csrq = csrq == null ? null : csrq.trim();
    }

    public String getCssj() {
        return cssj;
    }

    public void setCssj(String cssj) {
        this.cssj = cssj == null ? null : cssj.trim();
    }

    public String getCsdgj() {
        return csdgj;
    }

    public void setCsdgj(String csdgj) {
        this.csdgj = csdgj == null ? null : csdgj.trim();
    }

    public String getCsdssx() {
        return csdssx;
    }

    public void setCsdssx(String csdssx) {
        this.csdssx = csdssx == null ? null : csdssx.trim();
    }

    public String getCsdxz() {
        return csdxz;
    }

    public void setCsdxz(String csdxz) {
        this.csdxz = csdxz == null ? null : csdxz.trim();
    }

    public String getJhryhm() {
        return jhryhm;
    }

    public void setJhryhm(String jhryhm) {
        this.jhryhm = jhryhm == null ? null : jhryhm.trim();
    }

    public String getJhryxm() {
        return jhryxm;
    }

    public void setJhryxm(String jhryxm) {
        this.jhryxm = jhryxm == null ? null : jhryxm.trim();
    }

    public String getJhrygx() {
        return jhrygx;
    }

    public void setJhrygx(String jhrygx) {
        this.jhrygx = jhrygx == null ? null : jhrygx.trim();
    }

    public String getJhrehm() {
        return jhrehm;
    }

    public void setJhrehm(String jhrehm) {
        this.jhrehm = jhrehm == null ? null : jhrehm.trim();
    }

    public String getJhrexm() {
        return jhrexm;
    }

    public void setJhrexm(String jhrexm) {
        this.jhrexm = jhrexm == null ? null : jhrexm.trim();
    }

    public String getJhregx() {
        return jhregx;
    }

    public void setJhregx(String jhregx) {
        this.jhregx = jhregx == null ? null : jhregx.trim();
    }

    public String getFqhm() {
        return fqhm;
    }

    public void setFqhm(String fqhm) {
        this.fqhm = fqhm == null ? null : fqhm.trim();
    }

    public String getFqxm() {
        return fqxm;
    }

    public void setFqxm(String fqxm) {
        this.fqxm = fqxm == null ? null : fqxm.trim();
    }

    public String getMqhm() {
        return mqhm;
    }

    public void setMqhm(String mqhm) {
        this.mqhm = mqhm == null ? null : mqhm.trim();
    }

    public String getMqxm() {
        return mqxm;
    }

    public void setMqxm(String mqxm) {
        this.mqxm = mqxm == null ? null : mqxm.trim();
    }

    public String getJggj() {
        return jggj;
    }

    public void setJggj(String jggj) {
        this.jggj = jggj == null ? null : jggj.trim();
    }

    public String getJgssx() {
        return jgssx;
    }

    public void setJgssx(String jgssx) {
        this.jgssx = jgssx == null ? null : jgssx.trim();
    }

    public String getXx() {
        return xx;
    }

    public void setXx(String xx) {
        this.xx = xx == null ? null : xx.trim();
    }

    public String getXxjb() {
        return xxjb;
    }

    public void setXxjb(String xxjb) {
        this.xxjb = xxjb == null ? null : xxjb.trim();
    }

    public String getHh() {
        return hh;
    }

    public void setHh(String hh) {
        this.hh = hh == null ? null : hh.trim();
    }

    public String getCsdjlb() {
        return csdjlb;
    }

    public void setCsdjlb(String csdjlb) {
        this.csdjlb = csdjlb == null ? null : csdjlb.trim();
    }

    public String getCszmbh() {
        return cszmbh;
    }

    public void setCszmbh(String cszmbh) {
        this.cszmbh = cszmbh == null ? null : cszmbh.trim();
    }

    public String getSbrgmsfhm() {
        return sbrgmsfhm;
    }

    public void setSbrgmsfhm(String sbrgmsfhm) {
        this.sbrgmsfhm = sbrgmsfhm == null ? null : sbrgmsfhm.trim();
    }

    public String getSbrxm() {
        return sbrxm;
    }

    public void setSbrxm(String sbrxm) {
        this.sbrxm = sbrxm == null ? null : sbrxm.trim();
    }

    public String getSbrq() {
        return sbrq;
    }

    public void setSbrq(String sbrq) {
        this.sbrq = sbrq == null ? null : sbrq.trim();
    }

    public String getSblb() {
        return sblb;
    }

    public void setSblb(String sblb) {
        this.sblb = sblb == null ? null : sblb.trim();
    }

    public String getSbly() {
        return sbly;
    }

    public void setSbly(String sbly) {
        this.sbly = sbly == null ? null : sbly.trim();
    }

    public String getSldwjgdm() {
        return sldwjgdm;
    }

    public void setSldwjgdm(String sldwjgdm) {
        this.sldwjgdm = sldwjgdm == null ? null : sldwjgdm.trim();
    }

    public String getSldwmc() {
        return sldwmc;
    }

    public void setSldwmc(String sldwmc) {
        this.sldwmc = sldwmc == null ? null : sldwmc.trim();
    }

    public String getSlrxm() {
        return slrxm;
    }

    public void setSlrxm(String slrxm) {
        this.slrxm = slrxm == null ? null : slrxm.trim();
    }

    public String getSlsj() {
        return slsj;
    }

    public void setSlsj(String slsj) {
        this.slsj = slsj == null ? null : slsj.trim();
    }

    public Long getRkxlh() {
        return rkxlh;
    }

    public void setRkxlh(Long rkxlh) {
        this.rkxlh = rkxlh;
    }

    public String getSsxq() {
        return ssxq;
    }

    public void setSsxq(String ssxq) {
        this.ssxq = ssxq == null ? null : ssxq.trim();
    }

    public String getPcsjgdm() {
        return pcsjgdm;
    }

    public void setPcsjgdm(String pcsjgdm) {
        this.pcsjgdm = pcsjgdm == null ? null : pcsjgdm.trim();
    }

    public String getPcsdm() {
        return pcsdm;
    }

    public void setPcsdm(String pcsdm) {
        this.pcsdm = pcsdm == null ? null : pcsdm.trim();
    }

    public String getXzjddm() {
        return xzjddm;
    }

    public void setXzjddm(String xzjddm) {
        this.xzjddm = xzjddm == null ? null : xzjddm.trim();
    }

    public String getSpqk() {
        return spqk;
    }

    public void setSpqk(String spqk) {
        this.spqk = spqk == null ? null : spqk.trim();
    }

    public String getSpdwjgdm() {
        return spdwjgdm;
    }

    public void setSpdwjgdm(String spdwjgdm) {
        this.spdwjgdm = spdwjgdm == null ? null : spdwjgdm.trim();
    }

    public String getSpdwmc() {
        return spdwmc;
    }

    public void setSpdwmc(String spdwmc) {
        this.spdwmc = spdwmc == null ? null : spdwmc.trim();
    }

    public String getSprxm() {
        return sprxm;
    }

    public void setSprxm(String sprxm) {
        this.sprxm = sprxm == null ? null : sprxm.trim();
    }

    public String getSpsj() {
        return spsj;
    }

    public void setSpsj(String spsj) {
        this.spsj = spsj == null ? null : spsj.trim();
    }

    public String getSpjg() {
        return spjg;
    }

    public void setSpjg(String spjg) {
        this.spjg = spjg == null ? null : spjg.trim();
    }

    public String getLxdh() {
        return lxdh;
    }

    public void setLxdh(String lxdh) {
        this.lxdh = lxdh == null ? null : lxdh.trim();
    }

    public String getShqk() {
        return shqk;
    }

    public void setShqk(String shqk) {
        this.shqk = shqk == null ? null : shqk.trim();
    }

    public String getShdwjgdm() {
        return shdwjgdm;
    }

    public void setShdwjgdm(String shdwjgdm) {
        this.shdwjgdm = shdwjgdm == null ? null : shdwjgdm.trim();
    }

    public String getShdwmc() {
        return shdwmc;
    }

    public void setShdwmc(String shdwmc) {
        this.shdwmc = shdwmc == null ? null : shdwmc.trim();
    }

    public String getShrxm() {
        return shrxm;
    }

    public void setShrxm(String shrxm) {
        this.shrxm = shrxm == null ? null : shrxm.trim();
    }

    public String getShrq() {
        return shrq;
    }

    public void setShrq(String shrq) {
        this.shrq = shrq == null ? null : shrq.trim();
    }

    public Short getBlqk() {
        return blqk;
    }

    public void setBlqk(Short blqk) {
        this.blqk = blqk;
    }

    public String getQsrq() {
        return qsrq;
    }

    public void setQsrq(String qsrq) {
        this.qsrq = qsrq == null ? null : qsrq.trim();
    }

    public Short getShspzt() {
        return shspzt;
    }

    public void setShspzt(Short shspzt) {
        this.shspzt = shspzt;
    }

    public String getYwlx() {
        return ywlx;
    }

    public void setYwlx(String ywlx) {
        this.ywlx = ywlx == null ? null : ywlx.trim();
    }

    public String getShsplc() {
        return shsplc;
    }

    public void setShsplc(String shsplc) {
        this.shsplc = shsplc == null ? null : shsplc.trim();
    }

    public String getSpyyms() {
        return spyyms;
    }

    public void setSpyyms(String spyyms) {
        this.spyyms = spyyms == null ? null : spyyms.trim();
    }

    public String getSpbh() {
        return spbh;
    }

    public void setSpbh(String spbh) {
        this.spbh = spbh == null ? null : spbh.trim();
    }

    public String getSpjb() {
        return spjb;
    }

    public void setSpjb(String spjb) {
        this.spjb = spjb == null ? null : spjb.trim();
    }

    public Long getSppzxlh() {
        return sppzxlh;
    }

    public void setSppzxlh(Long sppzxlh) {
        this.sppzxlh = sppzxlh;
    }

    public String getBz() {
        return bz;
    }

    public void setBz(String bz) {
        this.bz = bz == null ? null : bz.trim();
    }

    public String getShspyj() {
        return shspyj;
    }

    public void setShspyj(String shspyj) {
        this.shspyj = shspyj == null ? null : shspyj.trim();
    }

    public String getDzbm() {
        return dzbm;
    }

    public void setDzbm(String dzbm) {
        this.dzbm = dzbm == null ? null : dzbm.trim();
    }

    public String getYsqrgx() {
        return ysqrgx;
    }

    public void setYsqrgx(String ysqrgx) {
        this.ysqrgx = ysqrgx == null ? null : ysqrgx.trim();
    }

    public String getXmpy() {
        return xmpy;
    }

    public void setXmpy(String xmpy) {
        this.xmpy = xmpy == null ? null : xmpy.trim();
    }

    public String getYwblsj() {
        return ywblsj;
    }

    public void setYwblsj(String ywblsj) {
        this.ywblsj = ywblsj == null ? null : ywblsj.trim();
    }

    public String getFxjdm() {
        return fxjdm;
    }

    public void setFxjdm(String fxjdm) {
        this.fxjdm = fxjdm == null ? null : fxjdm.trim();
    }

    public String getSlrid() {
        return slrid;
    }

    public void setSlrid(String slrid) {
        this.slrid = slrid == null ? null : slrid.trim();
    }

    public String getSprid() {
        return sprid;
    }

    public void setSprid(String sprid) {
        this.sprid = sprid == null ? null : sprid.trim();
    }

    public String getShrid() {
        return shrid;
    }

    public void setShrid(String shrid) {
        this.shrid = shrid == null ? null : shrid.trim();
    }

    public String getDqm() {
        return dqm;
    }

    public void setDqm(String dqm) {
        this.dqm = dqm == null ? null : dqm.trim();
    }

    public String getBdywswbh() {
        return bdywswbh;
    }

    public void setBdywswbh(String bdywswbh) {
        this.bdywswbh = bdywswbh == null ? null : bdywswbh.trim();
    }

    public String getOspjb() {
        return ospjb;
    }

    public void setOspjb(String ospjb) {
        this.ospjb = ospjb == null ? null : ospjb.trim();
    }

    public Short getQctbs() {
        return qctbs;
    }

    public void setQctbs(Short qctbs) {
        this.qctbs = qctbs;
    }

    public String getHzXm() {
        return hzXm;
    }

    public void setHzXm(String hzXm) {
        this.hzXm = hzXm == null ? null : hzXm.trim();
    }

    public String getHzGmsfhm() {
        return hzGmsfhm;
    }

    public void setHzGmsfhm(String hzGmsfhm) {
        this.hzGmsfhm = hzGmsfhm == null ? null : hzGmsfhm.trim();
    }
}